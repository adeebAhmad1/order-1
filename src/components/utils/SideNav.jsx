...
class SideNav extends Component {
  ...
  componentDidMount() {
    ...
    db.collection("collections").onSnapshot(snapShot => {
      ...
      if(collections.length === 0) db.collection("collections").add({name: "todos"}).then(()=>console.log("SUCCESS"))
  ...
  deleteCollection = (e)=>{
    ...
    db.collection(target.name).get().then(snapShot=>{
      const boardIds = []
      snapShot.forEach(doc=>{
        boardIds.push(doc.id)
      })
      boardIds.forEach(el=>{
        db.collection(target.name).doc(el).delete().then(()=>{
          db.collection("collections").doc(target.id).delete().then(()=>{
  ...
  showItems = () =>
    this.state.collections.map((el, i) => (
      <li key={i} className={this.state.board === el.name ? "item-active" : ""}>
        ...
        {this.props.page === "home" ? "" : el.name === "todos" ? "" : <i data-name={el.name} data-id={el.id} onClick={this.deleteCollection} className="fas fa-trash-alt trash-icon"></i>}
      </li> 
    ));
  render() {
    return (
              ...
              {!(this.props.page === "home") ? (
                <li>
                  <Link to="/admin_panel/add_board">
                    <i className="fas fa-plus-circle"> <span>Add Board</span></i>&nbsp;
                  </Link>
                </li>
              ) : ("")}
            </ul>
